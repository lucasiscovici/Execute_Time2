...
// ---------------------------------------------------------------------------
//
// Execution timings with timer:
// display when a cell was last executed, and how long it took to run
// A double click on the timing box makes it disappear with timer
...
define([
    ...
    var options = {
        ...
        template: {
            ...
            queued: 'execution queued ${start_time} (${duration})',
        },
        timeInterval:{},
    };
        ...
        CodeCell.prototype.get_callbacks = function () {
            ...
            callbacks.shell.reply = function (msg) {
                if (msg.msg_type === 'execute_reply') {
                    if (options.with_timer) {
                        clearInterval(options.timeInterval[cell.metadata.ExecuteTime2.id]);
                        delete options.timeInterval[cell.metadata.ExecuteTime2.id];
                    }
    ...
    function clear_timing_data (cells) {
        cells.forEach(function (cell, idx, arr) {
            delete cell.metadata.ExecuteTime2;
    ...
    function excute_codecell_callback (evt, data) {
        var cell = data.cell;
        var ID=makeid(10);
        cell.metadata.ExecuteTime2 = {start_time: moment().toISOString(),id:ID};
        if (options.with_timer) {
            var TI=setInterval(function(){
                update_timing_area(cell);
            },1000);
            options.timeInterval[ID]=TI;
        }
    ...
    function update_timing_area (cell) {
        if (! (cell instanceof CodeCell) ||
                 !cell.metadata.ExecuteTime2 ||
                 !cell.metadata.ExecuteTime2.start_time) {
            return $();
        }
        ...
        var start_time = moment(cell.metadata.ExecuteTime2.start_time),
              end_time = cell.metadata.ExecuteTime2.end_time;
        var msg = options.template[end_time ? 'executed' : 'queued'];
        msg = msg.replace('${start_time}', format_moment(start_time));
        if (! end_time){
            var now = moment().toISOString();
            var exec_time2 = -start_time.diff(now);
            msg = msg.replace("${duration}", humanized_duration(exec_time2));
        }
        if (end_time) {
